...
@section('head')
  <link rel="stylesheet" href="/css/cust-profile.css">
@endsection
...
<form action="{{ route('customer.profile') }}" method="POST">
  ...
  <div class="profile-section" id="personal-info-section">
    ...
    <div class="detail-row">
      ...
      <span class="static-value" id="name-static">{{ $customer->name }}</span>
      <input type="text" class="editable-field" id="name-edit" name="name" placeholder="Your full name" value="{{ $customer->name }}" required>
    </div>

    <div class="detail-row">
      <label for="email">Email Address</label>
      <span class="static-value" id="email-static">{{ $customer->email }}</span>
      <input type="email" class="editable-field" id="email-edit" name="email" placeholder="you@example.com" value="{{ $customer->email }}" required>
    </div>

    <div class="detail-row">
      <label for="phone">Phone Number</label>
      <span class="static-value" id="phone-static">{{ $customer->phone }}</span>
      <input type="text" class="editable-field" id="phone-edit" name="phone" placeholder="e.g. 012-3456789" value="{{ $customer->phone }}" required>
    </div>
